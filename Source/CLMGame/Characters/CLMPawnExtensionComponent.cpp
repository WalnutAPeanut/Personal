...
void UCLMPawnExtensionComponent::BeginPlay()
{
	...
	//      - CLMPawnExtensionComponent의 경우, 모든 Actor의 Feature 상태 변화에 대해 OnActorInitStateChanged()가 호출됨
...
void UCLMPawnExtensionComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	...
	{
		// CLMPawnExtensionComponent는 다른 Feature Component들의 상태가 DataAvailable를 관찰하여, Sync를 맞추는 구간이 있었다 (CanChangeInitState)
...
bool UCLMPawnExtensionComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	...
	{
		// PawnData는 GameMode SpawnDefaultPawnAtTransform_Implementation 에서 세팅이 된다.
		if (!PawnData)
		{
			return false;
		}
