...
UCLMCameraComponent::UCLMCameraComponent(const FObjectInitializer& ObjectInitializer)
	:Super(ObjectInitializer), CameraModeStack(nullptr)
{
}

void UCLMCameraComponent::OnRegister()
{
	Super::OnRegister();
	...
	if (!CameraModeStack)
	{
		...
		CameraModeStack = NewObject<UCLMCameraModeStack>(this);
	}
}

void UCLMCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView)
{
	...
	FCLMCameraModeView CameraModeView;
...
void UCLMCameraComponent::UpdateCameraModes()
{
	check(CameraModeStack);
	...
	{
		if (TSubclassOf<UCLMCameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
		{
